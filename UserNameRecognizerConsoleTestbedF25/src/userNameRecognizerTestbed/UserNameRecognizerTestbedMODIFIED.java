...
public class UserNameRecognizerTestbedMODIFIED{
	...
	public static void main(String[] args) {
		...
		while (keyboard.hasNextLine()) {
			...
			// Input has been provided, let's see if it is a valid date or not
 			String errMessage = UserNameRecognizerMODIFIED.checkForValidUserName(inputLine);
			...
			if (errMessage != "") {
				...
				if (UserNameRecognizerMODIFIED.userNameRecognizerIndexofError <= -1) return;	// Should never happen
				...
				System.out.println(inputLine.substring(0,UserNameRecognizerMODIFIED.userNameRecognizerIndexofError) + "\u21EB");
			}
			else {
